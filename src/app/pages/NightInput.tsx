    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center pt-1 pb-1 shrink-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 12 }}>📅 3月17日 · 周二</span>
      ...
      {entryCount > 0 && (
        <div className="absolute z-10" style={{ top: "27%", left: "50%", transform: "translateX(-50%)" }}>
          <span style={{ color: "rgba(160,200,255,0.3)", fontSize: 9, letterSpacing: 1 }}>
            ✦ 已点亮 {Math.min(entryCount, 5)}/5 组星座
          </span>
        </div>
      )}
        ...
        <div className="absolute z-30 left-1/2" style={{ top: "35%", transform: "translateX(-50%)", animation: "fadeInUp 0.4s ease" }}>
          <LiquidGlass mode="night" borderRadius={16} intensity="soft">
            <div className="flex items-center gap-2" style={{ padding: "8px 16px" }}>
              ...
              <span style={{ color: "rgba(180,220,255,0.9)", fontSize: 12 }}>新的星辰已点亮</span>
            </div>
          </LiquidGlass>
        </div>
      )}
        ...
        <LiquidGlass mode="night" borderRadius={22} intensity="medium">
          <div style={{ padding: "20px 20px 16px 20px" }}>
            ...
            <div className="flex gap-2 mb-3">
              {["情绪", "色彩", "光影"].map((tag) => (
                <span key={tag} style={{ background: "rgba(80,130,220,0.2)", border: "1px solid rgba(80,130,220,0.3)", color: "rgba(140,190,255,0.7)", fontSize: 11, padding: "2px 10px", borderRadius: 20 }}>
                  {tag}
                </span>
              ))}
            </div>
            <div className="flex items-center justify-between">
              <button onClick={() => onNavigate("night-camera")} style={{ color: "rgba(255,255,255,0.45)", padding: 6 }} className="flex items-center gap-1 transition-opacity hover:opacity-80">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                  <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" stroke="rgba(255,255,255,0.5)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  <circle cx="12" cy="13" r="4" stroke="rgba(255,255,255,0.5)" strokeWidth="1.5" />
                </svg>
              </button>
